...
def run_global_attack(
    ...
    miss_diagnostics_enabled = bool(
        getattr(adversary, "miss_diagnostics_enabled", False)
    )
    requires_fresh_attack = rq1_enabled or miss_diagnostics_enabled

    if requires_fresh_attack:
        # Per-epoch diagnostics, counterfactual probes, and injection
        # interventions cannot be reconstructed from a cached final graph.
        pert_adj = None
        pert_attr = None
        logging.info(
            "Attack diagnostics/intervention enabled: bypass perturbation cache."
        )
    else:
    ...
    if pert_adj is not None and pert_attr is not None:
        ...
        if n_perturbations > 0 and not requires_fresh_attack:
