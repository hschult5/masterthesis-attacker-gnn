...
class PRBCD(SparseAttack):
    ...
    def __init__(
            ...
            # RQ1 (legacy diagnostics; preserved for compatibility)
            rq1_enabled: bool = False,
            rq1_is_reference: bool = False,
            rq1_sampling_seed: Optional[int] = None,

            # Opt-in miss-condition diagnostics. All defaults preserve the
            # original PRBCD execution path and outputs.
            miss_diagnostics_enabled: bool = False,
            miss_sampling_seed: Optional[int] = None,
            miss_checkpoint_epochs: Optional[List[int]] = None,
            miss_store_checkpoint_blocks: bool = True,
            miss_store_resample_edge_details: bool = False,
            miss_track_edge_signals: bool = True,
            miss_retention_policy: str = "native",
            miss_probe_linear_ids: Optional[List[int]] = None,
            # Legacy compatibility only; discrete probe contexts are no longer used.
            miss_probe_context_mode: str = "topk_budget",
            miss_probe_max_candidates: int = 500,
            miss_injection_linear_ids: Optional[List[int]] = None,
            miss_injection_epoch: int = -1,
            miss_injection_initial_weight: Optional[float] = None,
        ...
        # Miss-condition diagnostics are intentionally independent from
        # `use_cert`. In particular, use_cert="none" remains the standard
        # random-block PRBCD algorithm. Diagnostics only observe that run or,
        # when explicitly requested, perform a probe/injection intervention.
        self.miss_sampling_seed = (
            int(miss_sampling_seed)
            if miss_sampling_seed is not None
            else None
        )
        self.miss_checkpoint_epochs = sorted({
            int(epoch)
            for epoch in (miss_checkpoint_epochs or [])
            if int(epoch) >= 0
        })
        self.miss_store_checkpoint_blocks = bool(
            miss_store_checkpoint_blocks
        )
        self.miss_store_resample_edge_details = bool(
            miss_store_resample_edge_details
        )
        self.miss_track_edge_signals = bool(miss_track_edge_signals)
        self.miss_retention_policy = str(miss_retention_policy)
        if self.miss_retention_policy not in {
            "native",
            "random_keep",
            "reverse_keep",
            "full_resample",
            ...
            raise ValueError(
                "miss_retention_policy must be one of: native, "
                "random_keep, reverse_keep, full_resample."
            )
        if self.miss_retention_policy != "native":
            miss_diagnostics_enabled = True
        self.miss_probe_linear_ids = [
            int(edge_id)
            for edge_id in (miss_probe_linear_ids or [])
        ][:max(0, int(miss_probe_max_candidates))]
        self.miss_probe_context_mode = str(miss_probe_context_mode)
        self.miss_probe_max_candidates = max(
            0,
            int(miss_probe_max_candidates),
        )
        self.miss_injection_linear_ids = [
            int(edge_id)
            for edge_id in (miss_injection_linear_ids or [])
        ]
        self.miss_injection_epoch = int(miss_injection_epoch)
        self.miss_injection_initial_weight = (
            float(miss_injection_initial_weight)
            if miss_injection_initial_weight is not None
            else None
        )
        self.miss_diagnostics_enabled = bool(
            miss_diagnostics_enabled
            or self.miss_probe_linear_ids
            or self.miss_injection_linear_ids
        )
        self._miss_injection_applied = False
    ...
    def _attack(self, ads_mode, graph, n_perturbations, semi=False, use_cert="none", grid_radii: Optional[np.ndarray] = None, grid_binary_class: Optional[np.ndarray] = None, **kwargs):
        ...
        if self.miss_diagnostics_enabled and self.miss_sampling_seed is not None:
            attack_sampling_seed = int(self.miss_sampling_seed)
        ...
        if (
            ...
            or self.miss_diagnostics_enabled
        ):
        ...
        if self.miss_diagnostics_enabled:
            self._miss_initialize_statistics(
        ...
        if self.miss_diagnostics_enabled:
            self._miss_record_initial_block()
        ...
        for epoch in tqdm(range(self.epochs)):
            if self.miss_diagnostics_enabled:
                self._miss_apply_injection(epoch=epoch)
                ...
                if self.rq1_is_reference:
                    ...
                    if use_cert in (
                        ...
                        if use_cert == "accuracy_drop_selector_with_resampling":
                            miss_before_selector = (
                                self.current_search_space.detach().clone()
                                if self.miss_diagnostics_enabled
                                else None
                            )
                            ...
                            if self.miss_diagnostics_enabled:
                                self._miss_record_external_resample(
                                    epoch=epoch,
                                    before_ids=miss_before_selector,
                                    policy="linkpred_threshold",
                                )
                        else:
        ...
        if self.miss_diagnostics_enabled:
            self._miss_finalize_statistics()
    ...
    # ============================================================
    # Opt-in miss-condition diagnostics
    # ============================================================

    def _miss_initialize_statistics(
            self,
            n_perturbations: int,
            sampling_seed: int,
            use_cert: str,
    ):
        """Allocate diagnostics only when explicitly requested."""
        self._miss_n_perturbations = int(n_perturbations)
        self._miss_retention_generator = torch.Generator(device=self.device)
        self._miss_retention_generator.manual_seed(int(sampling_seed) + 99173)
        self._miss_ever_seen = torch.zeros(
            self.n_possible_edges,
            dtype=torch.bool,
            device=self.device,
        )
        self._miss_first_seen = torch.full(
            (self.n_possible_edges,),
            -1,
            dtype=torch.int32,
            device=self.device,
        )
        self._miss_last_seen = torch.full(
            (self.n_possible_edges,),
            -1,
            dtype=torch.int32,
            device=self.device,
        )
        self._miss_exposure_count = torch.zeros(
            self.n_possible_edges,
            dtype=torch.int32,
            device=self.device,
        )

        if self.miss_track_edge_signals:
            self._miss_max_weight = torch.zeros(
                self.n_possible_edges,
                dtype=torch.float32,
                device=self.device,
            )
            self._miss_positive_gradient_sum = torch.zeros(
                self.n_possible_edges,
                dtype=torch.float32,
                device=self.device,
            )
        else:
            self._miss_max_weight = None
            self._miss_positive_gradient_sum = None

        # Clean-edge membership is needed only to label probe actions.
        self._miss_clean_present = torch.zeros(
            self.n_possible_edges,
            dtype=torch.bool,
            device=self.device,
        )
        clean_rows = self.edge_index[0].to(self.device).long()
        clean_cols = self.edge_index[1].to(self.device).long()
        non_self = clean_rows != clean_cols
        if non_self.any():
            if self.make_undirected:
                clean_u = torch.minimum(
                    clean_rows[non_self],
                    clean_cols[non_self],
                )
                clean_v = torch.maximum(
                    clean_rows[non_self],
                    clean_cols[non_self],
                )
                clean_linear = PRBCD.triu_idx_to_linear_idx(
                    self.n,
                    torch.stack([clean_u, clean_v]),
                ).long()
            else:
                clean_linear = PRBCD.full_to_linear_idx(
                    self.n,
                    torch.stack([
                        clean_rows[non_self],
                        clean_cols[non_self],
            ...
            self._miss_clean_present[torch.unique(clean_linear)] = True

        valid_probe_ids = sorted({
            edge_id
            for edge_id in self.miss_probe_linear_ids
            if 0 <= edge_id < self.n_possible_edges
        })[:self.miss_probe_max_candidates]
        self._miss_probe_ids = torch.tensor(
            valid_probe_ids,
            dtype=torch.long,
            device=self.device,
        )

        valid_injection_ids = sorted({
            edge_id
            for edge_id in self.miss_injection_linear_ids
            if 0 <= edge_id < self.n_possible_edges
        })
        self._miss_injection_ids = torch.tensor(
            valid_injection_ids,
            dtype=torch.long,
            device=self.device,
        )

        self.attack_statistics["miss_diagnostics"] = {
            "metadata": {
                "enabled": True,
                "block_size": int(self.block_size),
                "n_perturbations": int(n_perturbations),
                ...
                "epochs_resampling": int(self.epochs_resampling),
                "checkpoint_epochs": list(self.miss_checkpoint_epochs),
                "probe_effect_mode": "one_step_full_block_replacement",
                "probe_gradient_mode": "isolated_single_swap",
                "probe_candidates_per_backward": 1,
                "probe_replacement_rule": "minimum_current_block_weight",
                "probe_step_comparison": "candidate_post_step_minus_baseline_post_step",
                "retention_policy": str(self.miss_retention_policy),
                "n_probe_edges": int(self._miss_probe_ids.numel()),
                "injection_epoch": int(self.miss_injection_epoch),
                "n_injection_edges": int(self._miss_injection_ids.numel()),
            },
            "initial_block": None,
            "checkpoints": {},
            "resample_events": [],
            "probe_results": [],
            "injection_events": [],
            "ever_seen_ids": torch.empty(0, dtype=torch.long),
            "final_linear_ids": torch.empty(0, dtype=torch.long),
            "edge_summary": {},
        }

    def _miss_mark_seen(self, edge_ids: torch.Tensor, epoch: int):
        if not self.miss_diagnostics_enabled or edge_ids is None:
            return
        edge_ids = torch.unique(edge_ids.detach().to(self.device).long())
        if edge_ids.numel() == 0:
            return
        new_mask = ~self._miss_ever_seen[edge_ids]
        new_ids = edge_ids[new_mask]
        if new_ids.numel() > 0:
            self._miss_ever_seen[new_ids] = True
            self._miss_first_seen[new_ids] = int(epoch)

    def _miss_record_initial_block(self):
        ids = self.current_search_space.detach().to(self.device).long()
        self._miss_mark_seen(ids, epoch=0)
        stats = self.attack_statistics["miss_diagnostics"]
        stats["initial_block"] = ids.detach().cpu().clone()
        stats["metadata"]["initial_actual_block_size"] = int(ids.numel())
        stats["metadata"]["initial_raw_draw_count"] = int(self.block_size)

    def _miss_update_edge_signals(
            self,
            epoch: int,
        ...
        ids = current_ids.detach().to(self.device).long()
        weights = current_weights.detach().to(self.device).float()
        self._miss_mark_seen(ids, epoch=epoch)
        self._miss_last_seen[ids] = int(epoch)
        self._miss_exposure_count.index_add_(
            0,
            ids,
            torch.ones_like(ids, dtype=torch.int32),
        )
        if self.miss_track_edge_signals:
            self._miss_max_weight[ids] = torch.maximum(
                self._miss_max_weight[ids],
                weights,
            )
            if gradient is not None and gradient.numel() == ids.numel():
                self._miss_positive_gradient_sum.index_add_(
                    0,
                    ids,
                    gradient.detach().to(self.device).float().clamp_min(0.0),
                )

    def _miss_build_adj_from_linear_ids(
            self,
            linear_ids: torch.Tensor,
            flip_weights: torch.Tensor,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Build A xor p without mutating the running PRBCD state."""
        linear_ids = linear_ids.to(self.device).long()
        flip_weights = flip_weights.to(self.device).float()
        clean_index = self.edge_index.to(self.device)
        clean_weight = (
            self.edge_weight.to(self.device).float()
            if getattr(self, "edge_weight", None) is not None
            else torch.ones(clean_index.size(1), device=self.device)
        )
        if linear_ids.numel() == 0:
            return clean_index, clean_weight

        if self.make_undirected:
            modified_index = PRBCD.linear_to_triu_idx(
                self.n,
                linear_ids,
            )
            modified_index, modified_weight = utils.to_symmetric(
                modified_index,
                flip_weights,
                self.n,
            )
        else:
            modified_index = PRBCD.linear_to_full_idx(
                self.n,
                linear_ids,
            )
            keep = modified_index[0] != modified_index[1]
            modified_index = modified_index[:, keep]
            modified_weight = flip_weights[keep]

        edge_index = torch.cat([clean_index, modified_index], dim=-1)
        edge_weight = torch.cat([clean_weight, modified_weight])
        edge_index, edge_weight = torch_sparse.coalesce(
            edge_index,
            edge_weight,
            m=self.n,
    ...
    def _miss_release_model_cache(self):
        if hasattr(self.attacked_model, "release_cache"):
            self.attacked_model.release_cache()

    def _miss_evaluate_state(
            self,
            linear_ids: torch.Tensor,
            flip_weights: torch.Tensor,
    ) -> Tuple[float, float]:
        self._miss_release_model_cache()
        edge_index, edge_weight = self._miss_build_adj_from_linear_ids(
            linear_ids,
            flip_weights,
        )
        logits = self._get_logits(self.attr, edge_index, edge_weight)
        ...
        acc = utils.accuracy(logits, self.labels, self.idx_attack)
        return float(loss.detach().cpu().item()), float(acc)

    def _miss_simulate_prbcd_step(
            self,
            linear_ids: torch.Tensor,
        ...
        """Simulate one normal relaxed PRBCD step without mutating the replay.

        The branch uses the same attack loss, learning-rate schedule, gradient
        ascent update, epsilon floor, and relaxed L0 projection as the main
        PRBCD loop. The returned post-step loss is the loss PRBCD would see on
        the following forward pass from the projected weights.
        """
        linear_ids = linear_ids.detach().to(self.device).long().flatten()
        initial_weights = (
            initial_weights.detach().to(self.device).float().flatten()
        )
        if linear_ids.numel() != initial_weights.numel():
            raise ValueError(
                "linear_ids and initial_weights must have equal length."
            )
        if linear_ids.numel() == 0:
            raise ValueError("Cannot simulate a PRBCD step on an empty block.")

        branch_weights = initial_weights.clone().requires_grad_(True)
        self._miss_release_model_cache()
        ...
        with torch.enable_grad():
            ...
            logits = self._get_logits(self.attr, edge_index, edge_weight)
            pre_step_loss_tensor = self.calculate_loss(
                logits[self.idx_attack],
                self.labels[self.idx_attack],
            )
            gradient = utils.grad_with_checkpoint(
                pre_step_loss_tensor,
                branch_weights,
            )[0]
        ...
        learning_rate = lr_factor / np.sqrt(
            max(0, int(step_epoch) - self.epochs_resampling) + 1
        )

        with torch.no_grad():
            updated_weights = (
                branch_weights.detach()
                + float(learning_rate) * gradient.detach()
            )
            updated_weights[updated_weights < self.eps] = self.eps
            projected_weights = Attack.project(
                int(n_perturbations),
                updated_weights,
                self.eps,
            ).detach()
            post_step_loss, post_step_accuracy = self._miss_evaluate_state(
                linear_ids,
                projected_weights,
            )
        ...
        result = {
            "pre_step_loss": float(
                pre_step_loss_tensor.detach().cpu().item()
            ),
            "post_step_loss": float(post_step_loss),
            "post_step_accuracy": float(post_step_accuracy),
            "gradient": gradient.detach(),
            "projected_weights": projected_weights.detach(),
            "learning_rate": float(learning_rate),
            "probability_mass_before": float(
                initial_weights.sum().detach().cpu().item()
            ),
            "probability_mass_after": float(
                projected_weights.sum().detach().cpu().item()
            ),
        }
    ...
    def _miss_record_checkpoint_and_probes(
            self,
            epoch: int,
            n_perturbations: int,
    ):
        """Record a checkpoint and run behavior-neutral one-step probes.

        For each probe absent from the current block, replace the minimum-weight
        block member with the probe initialized at epsilon. Every probe receives
        its own forward/backward pass on a block containing no other probes. Then
        simulate one identical PRBCD step for (a) the unchanged block and (b)
        that single-probe replacement block. The causal probe effect is candidate
        post-step loss minus baseline post-step loss. Neither branch mutates the
        running replay.
        """
        if int(epoch) not in set(self.miss_checkpoint_epochs):
            return

        stats = self.attack_statistics["miss_diagnostics"]
        current_ids = (
            self.current_search_space.detach().to(self.device).long()
        )
        current_weights = (
            self.perturbed_edge_weight.detach().to(self.device).float()
        )
        if current_ids.numel() == 0:
            raise RuntimeError(
                "Miss-condition checkpoint encountered an empty PRBCD block."
            )

        # Checkpoints are recorded after epoch t's update/projection. Therefore
        # the next hypothetical PRBCD update uses the epoch t+1 learning rate.
        one_step_epoch = int(epoch) + 1
        baseline_step = self._miss_simulate_prbcd_step(
            current_ids,
            current_weights,
            n_perturbations=int(n_perturbations),
            step_epoch=one_step_epoch,
        )

        replacement_drop_idx = int(torch.argmin(current_weights).item())
        replacement_drop_id = int(
            current_ids[replacement_drop_idx].item()
        )
        replacement_drop_weight = float(
            current_weights[replacement_drop_idx].item()
        )

        checkpoint = {
            "epoch": int(epoch),
            "actual_block_size": int(current_ids.numel()),
            "relaxed_loss": float(baseline_step["pre_step_loss"]),
            "one_step_epoch": int(one_step_epoch),
            "one_step_learning_rate": float(
                baseline_step["learning_rate"]
            ),
            "one_step_baseline_loss": float(
                baseline_step["post_step_loss"]
            ),
            "one_step_baseline_accuracy": float(
                baseline_step["post_step_accuracy"]
            ),
            "one_step_baseline_probability_mass_before": float(
                baseline_step["probability_mass_before"]
            ),
            "one_step_baseline_probability_mass_after": float(
                baseline_step["probability_mass_after"]
            ),
            "replacement_dropped_linear_id": int(replacement_drop_id),
            "replacement_dropped_weight": float(replacement_drop_weight),
            "replacement_rule": "minimum_current_block_weight",
        }
        if self.miss_store_checkpoint_blocks:
            checkpoint["current_search_space"] = (
                current_ids.detach().cpu().clone()
            )
            checkpoint["current_weights"] = (
                current_weights.detach().cpu().clone()
            )
            checkpoint["one_step_baseline_projected_weights"] = (
                baseline_step["projected_weights"].detach().cpu().clone()
            )
        stats["checkpoints"][int(epoch)] = checkpoint

        if self._miss_probe_ids.numel() == 0:
            self._miss_release_model_cache()
            return

        # A coverage-miss probe must be absent from the checkpoint block.
        probe_ids = self._miss_probe_ids[
            ~torch.isin(self._miss_probe_ids, current_ids)
        ]
        if probe_ids.numel() == 0:
            self._miss_release_model_cache()
            return
        ...
        keep_mask[replacement_drop_idx] = False
        retained_ids = current_ids[keep_mask]
        retained_weights = current_weights[keep_mask]

        for edge_id_tensor in probe_ids:
            edge_id = int(edge_id_tensor.item())
            candidate = edge_id_tensor.view(1)

            candidate_ids = torch.cat([retained_ids, candidate])
            candidate_initial_weights = torch.cat([
                retained_weights,
                torch.full(
                    (1,),
                    float(self.eps),
            ...
            if candidate_ids.numel() != current_ids.numel():
                raise RuntimeError(
                    "Single-swap probe changed the PRBCD block size."
                )
            if torch.unique(candidate_ids).numel() != candidate_ids.numel():
                raise RuntimeError(
                    "Single-swap probe block contains duplicate edge IDs."
                )

            # Keep the standard sorted linear-ID representation and preserve
            # alignment between IDs and weights.
            candidate_ids, order = torch.sort(candidate_ids)
            candidate_initial_weights = candidate_initial_weights[order]
            candidate_position = int(
                torch.nonzero(
                    candidate_ids == edge_id,
                    as_tuple=False,
                ).flatten()[0].item()
            )

            candidate_step = self._miss_simulate_prbcd_step(
                candidate_ids,
            ...
            pair = (
                PRBCD.linear_to_triu_idx(self.n, candidate)
                if self.make_undirected
                else PRBCD.linear_to_full_idx(self.n, candidate)
            )
            candidate_gradient = float(
                candidate_step["gradient"][candidate_position]
                .detach().cpu().item()
            )
            candidate_post_weight = float(
                candidate_step["projected_weights"][candidate_position]
                .detach().cpu().item()
            )

            stats["probe_results"].append({
                "epoch": int(epoch),
                "one_step_epoch": int(one_step_epoch),
                "linear_id": edge_id,
                "u": int(pair[0, 0].item()),
                ...
                "action": (
                    "delete"
                    if bool(self._miss_clean_present[edge_id].item())
                    else "add"
                ),
                "replacement_dropped_linear_id": int(
                    replacement_drop_id
                ),
                "replacement_dropped_weight": float(
                    replacement_drop_weight
                ),
                "candidate_initial_weight": float(self.eps),
                "probe_gradient_mode": "isolated_single_swap",
                "probe_candidates_in_context": 1,
                "probe_context_size": int(candidate_ids.numel()),
                "candidate_gradient_at_eps": float(candidate_gradient),
                # Preserve the familiar column name for downstream plots.
                "gradient_at_eps": float(candidate_gradient),
                "candidate_post_step_weight": float(candidate_post_weight),
                "one_step_learning_rate": float(
                    candidate_step["learning_rate"]
                ),
                "checkpoint_baseline_loss": float(
                    baseline_step["pre_step_loss"]
                ),
                "candidate_pre_step_loss": float(
                    candidate_step["pre_step_loss"]
                ),
                "one_step_baseline_loss": float(
                    baseline_step["post_step_loss"]
                ),
                "one_step_candidate_loss": float(
                    candidate_step["post_step_loss"]
                ),
                "one_step_delta_loss": float(
                    candidate_step["post_step_loss"]
                    - baseline_step["post_step_loss"]
                ),
                "one_step_baseline_accuracy": float(
                ...
                "one_step_candidate_accuracy": float(
                    candidate_step["post_step_accuracy"]
                ),
                "one_step_accuracy_drop": float(
                    baseline_step["post_step_accuracy"]
                    - candidate_step["post_step_accuracy"]
                ),
                "one_step_baseline_probability_mass": float(
                    baseline_step["probability_mass_after"]
                ),
                "one_step_candidate_probability_mass": float(
                    candidate_step["probability_mass_after"]
                ),
            })

            del candidate_step

        # Probe-specific forwards must not affect the replayed trajectory.
        self._miss_release_model_cache()

    @torch.no_grad()
    def _miss_apply_injection(self, epoch: int):
        if (
            self._miss_injection_applied
            or self._miss_injection_ids.numel() == 0
            or int(epoch) != int(self.miss_injection_epoch)
        ):
            return

        current_ids = self.current_search_space.detach().long()
        current_weights = self.perturbed_edge_weight.detach().float()
        insert_ids = self._miss_injection_ids[
            ~torch.isin(self._miss_injection_ids, current_ids)
        ]
        if insert_ids.numel() == 0:
            self._miss_injection_applied = True
            return

        n_replace = min(int(insert_ids.numel()), int(current_ids.numel()))
        insert_ids = insert_ids[:n_replace]
        lowest_idx = torch.argsort(current_weights)[:n_replace]
        keep_mask = torch.ones(
            current_ids.numel(),
            dtype=torch.bool,
            device=self.device,
        ...
        initial_weight = (
            float(self.miss_injection_initial_weight)
            if self.miss_injection_initial_weight is not None
            else float(self.eps)
        )
        new_ids = torch.cat([current_ids[keep_mask], insert_ids])
        ...
        self._miss_mark_seen(insert_ids, epoch=epoch)
        self.attack_statistics["miss_diagnostics"][
            "injection_events"
        ].append({
            "epoch": int(epoch),
            "inserted_ids": insert_ids.detach().cpu().clone(),
            "dropped_ids": dropped_ids.detach().cpu().clone(),
            "initial_weight": float(initial_weight),
        })
        self._miss_injection_applied = True

    def _miss_record_random_resample(
            self,
            epoch: Optional[int],
            before_ids: torch.Tensor,
            before_weights: torch.Tensor,
            before_gradient: Optional[torch.Tensor],
            kept_ids: torch.Tensor,
            dropped_ids: torch.Tensor,
            raw_sampled_ids: torch.Tensor,
            n_eps: int,
            raw_draw_count: int,
    ):
        after_ids = self.current_search_space.detach().clone()
        next_epoch = int(epoch) + 1 if epoch is not None else -1
        self._miss_mark_seen(after_ids, epoch=next_epoch)
        new_ids = after_ids[~torch.isin(after_ids, kept_ids)]
        event = {
            "epoch": int(epoch) if epoch is not None else None,
            "policy": str(self.miss_retention_policy),
            "before_size": int(before_ids.numel()),
            "after_size": int(after_ids.numel()),
            "n_eps": int(n_eps),
            "n_discard": int(dropped_ids.numel()),
            "n_keep": int(kept_ids.numel()),
            "raw_draw_count": int(raw_draw_count),
            "raw_unique_draw_count": int(torch.unique(raw_sampled_ids).numel()),
            "n_new_unique_in_after": int(new_ids.numel()),
        }
        if self.miss_store_resample_edge_details:
            event.update({
                "before_ids": before_ids.detach().cpu().clone(),
                "before_weights": before_weights.detach().cpu().clone(),
                "before_gradients": (
                    before_gradient.detach().cpu().clone()
                    if before_gradient is not None
                    else None
                ),
                "kept_ids": kept_ids.detach().cpu().clone(),
                "dropped_ids": dropped_ids.detach().cpu().clone(),
                "raw_sampled_ids": raw_sampled_ids.detach().cpu().clone(),
                "after_ids": after_ids.detach().cpu().clone(),
                "new_ids": new_ids.detach().cpu().clone(),
            })
        self.attack_statistics["miss_diagnostics"][
            "resample_events"
        ].append(event)

    def _miss_record_external_resample(
            self,
            epoch: int,
            before_ids: torch.Tensor,
            policy: str,
        ...
        after_ids = self.current_search_space.detach().clone()
        self._miss_mark_seen(after_ids, epoch=int(epoch) + 1)
        kept_ids = after_ids[torch.isin(after_ids, before_ids)]
        dropped_ids = before_ids[~torch.isin(before_ids, after_ids)]
        new_ids = after_ids[~torch.isin(after_ids, before_ids)]
        event = {
            "epoch": int(epoch),
            "policy": str(policy),
            "before_size": int(before_ids.numel()),
    ...
    def _miss_finalize_statistics(self):
        stats = self.attack_statistics["miss_diagnostics"]
        final_ids = self.current_search_space.detach().long()[
            self.perturbed_edge_weight.detach() > 0.5
        ]
        final_ids = torch.unique(final_ids, sorted=True)
        seen_ids = torch.nonzero(
            self._miss_ever_seen,
            as_tuple=False,
        ).flatten()
        stats["ever_seen_ids"] = seen_ids.detach().cpu().clone()
        stats["final_linear_ids"] = final_ids.detach().cpu().clone()
        edge_summary = {
            "linear_ids": seen_ids.detach().cpu().clone(),
            "first_seen_epoch": self._miss_first_seen[
                seen_ids
            ].detach().cpu().clone(),
            "last_seen_epoch": self._miss_last_seen[
                seen_ids
            ].detach().cpu().clone(),
            "exposure_count": self._miss_exposure_count[
                seen_ids
            ].detach().cpu().clone(),
            "is_final": torch.isin(
                seen_ids,
        ...
        if self.miss_track_edge_signals:
            edge_summary["max_weight"] = self._miss_max_weight[
                seen_ids
            ].detach().cpu().clone()
            edge_summary[
                "positive_gradient_sum"
            ] = self._miss_positive_gradient_sum[
                seen_ids
            ].detach().cpu().clone()
        stats["edge_summary"] = edge_summary
        stats["metadata"]["n_ever_seen"] = int(seen_ids.numel())
        stats["metadata"]["coverage_fraction"] = float(
            seen_ids.numel() / max(1, self.n_possible_edges)
        )
        stats["metadata"]["n_final"] = int(final_ids.numel())
        stats["metadata"]["total_raw_resample_draws"] = int(sum(
            event.get("raw_draw_count") or 0
            for event in stats["resample_events"]
        ))
    ...
    def resample_random_block(
            ...
            n_perturbations: int,
            mod_block_size: int,
            diagnostic_epoch: Optional[int] = None,
    ):
        """WeightOnly resampling with optional, behavior-neutral diagnostics."""
        if self.miss_diagnostics_enabled:
            before_ids = self.current_search_space.detach().clone()
            before_weights = self.perturbed_edge_weight.detach().clone()
            before_gradient = (
                self.gradient.detach().clone()
                if getattr(self, "gradient", None) is not None
                and self.gradient.numel() == before_ids.numel()
                else None
            )
        else:
            before_ids = None
        ...
        if self.keep_heuristic == 'WeightOnly':
            full_sorted_idx = torch.argsort(self.perturbed_edge_weight)
            n_eps = int(
                (self.perturbed_edge_weight <= self.eps).sum().item()
            )
            n_discard = n_eps
            # Native WeightOnly discards at least the lower half.
            if n_discard < full_sorted_idx.size(0) // 2:
                n_discard = full_sorted_idx.size(0) // 2
        else:
        ...
        policy = (
            self.miss_retention_policy
            if self.miss_diagnostics_enabled
            else "native"
        )
        n_keep = int(full_sorted_idx.numel()) - int(n_discard)
        if policy == "native":
            dropped_idx = full_sorted_idx[:n_discard]
            kept_idx = full_sorted_idx[n_discard:]
        elif policy == "reverse_keep":
            kept_idx = full_sorted_idx[:n_keep]
            dropped_idx = full_sorted_idx[n_keep:]
        elif policy == "random_keep":
            random_order = torch.randperm(
                full_sorted_idx.numel(),
                generator=self._miss_retention_generator,
                device=self.device,
            )
            kept_idx = random_order[:n_keep]
            dropped_idx = random_order[n_keep:]
        elif policy == "full_resample":
            kept_idx = torch.empty(0, dtype=torch.long, device=self.device)
            dropped_idx = torch.arange(
                full_sorted_idx.numel(),
                device=self.device,
            )
            n_discard = int(full_sorted_idx.numel())
        else:
            raise RuntimeError(f"Unknown retention policy: {policy}")
        if self.miss_diagnostics_enabled:
            dropped_ids = before_ids[dropped_idx]
            kept_ids = before_ids[kept_idx]
        else:
            dropped_ids = None
            kept_ids = None

        self.current_search_space = self.current_search_space[kept_idx]
        self.modified_edge_index = self.modified_edge_index[:, kept_idx]
        self.perturbed_edge_weight = self.perturbed_edge_weight[kept_idx]

        raw_draw_count = 0
        sampled_raw_parts = []

        # Sample until enough edges were drawn. This preserves the original
        # stopping rule and random-number calls exactly when diagnostics are off.
        for _ in range(self.max_final_samples):
            n_edges_resample = (
                int(mod_block_size)
                - int(self.current_search_space.size(0))
            )
            ...
            raw_draw_count += int(lin_index.numel())
            if self.miss_diagnostics_enabled:
                sampled_raw_parts.append(lin_index.detach().clone())
            ...
            if not self.make_undirected:
                ...
                self.modified_edge_index = self.modified_edge_index[
                    :, is_not_self_loop
                ]
                self.perturbed_edge_weight = self.perturbed_edge_weight[
                    is_not_self_loop
                ]
            ...
            if self.current_search_space.size(0) > n_perturbations:
                if self.miss_diagnostics_enabled:
                    raw_ids = (
                        torch.cat(sampled_raw_parts)
                        if sampled_raw_parts
                        else torch.empty(
                            0,
                            device=self.device,
                            dtype=torch.long,
                    ...
                    self._miss_record_random_resample(
                        epoch=diagnostic_epoch,
                        before_ids=before_ids,
                        before_weights=before_weights,
                        before_gradient=before_gradient,
                        kept_ids=kept_ids,
                        dropped_ids=dropped_ids,
                        raw_sampled_ids=raw_ids,
                        n_eps=n_eps,
                        raw_draw_count=raw_draw_count,
                    )
                return

        raise RuntimeError(
            'Sampling random block was not successfull. '
    ...
    def _append_attack_statistics(
        ...
        # ----------------------------------------------------------
        # Optional miss-condition diagnostics
        # ----------------------------------------------------------

        if self.miss_diagnostics_enabled:
            if epoch is not None:
                current_ids_for_miss = self.current_search_space.detach()
                current_weights_for_miss = self.perturbed_edge_weight.detach()
                self._miss_update_edge_signals(
                    epoch=int(epoch),
                    current_ids=current_ids_for_miss,
                    current_weights=current_weights_for_miss,
                    gradient=gradient,
                )
                self._miss_record_checkpoint_and_probes(
                    epoch=int(epoch),
                    n_perturbations=self._miss_n_perturbations,
                )
